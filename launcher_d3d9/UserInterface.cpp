...
const std::wstring& getText(const LanguageData& data, ID id) {
	...
	static auto myEnglish = loadMyCSF(GetModuleHandle(nullptr), MAKEINTRESOURCEW(BUILTIN_ENGLISH), (LPCWSTR)RT_RCDATA);
	static auto myChinese = loadMyCSF(GetModuleHandle(nullptr), MAKEINTRESOURCEW(BUILTIN_CHINESE), (LPCWSTR)RT_RCDATA);
			...
			case IDC_ENABLE_PROXY: {
				...
				if (checked) {
					...
					auto dllData = Windows::loadBinaryDataResource<char>(
						GetModuleHandle(nullptr), MAKEINTRESOURCEW(PROXY_DLL), (LPCWSTR)RT_RCDATA);
...
void aboutWindow(HWND parent, HICON icon, const LanguageData& languageData) {
	...
	auto handlers = ModalDialogBox::HandlerTable{};
	auto font = getNormalFont();
	auto ra3barData = loadBinaryDataResource<DWORD>(GetModuleHandle(nullptr), MAKEINTRESOURCEW(RA3BAR_LOGO), (LPCWSTR)RT_RCDATA);
