...
//
// d3d9 version: uses d3d9.dll proxy instead of winmm.dll.
// Differs from winmm-based launcher in:
//   - Registry override target: "d3d9" instead of "winmm"
//   - DLL location: game root (next to RA3.exe) instead of Data/
//   - Embedded resource: d3d9_proxy.dll instead of winmm_proxy.dll
//
// Copyright and attribution: this file is a derivative of the
// original RA3Bar launcher (see launcher/launcher.patch).
...
const std::wstring& getText(const LanguageData& data, ID id) {
	...
	static auto myEnglish = loadMyCSF(GetModuleHandle(nullptr), MAKEINTRESOURCEW(BUILTIN_ENGLISH), (LPCWSTR)(LPCWSTR)RT_RCDATA);
	static auto myChinese = loadMyCSF(GetModuleHandle(nullptr), MAKEINTRESOURCEW(BUILTIN_CHINESE), (LPCWSTR)(LPCWSTR)RT_RCDATA);
...
std::optional<LaunchOptions> runControlCenter(const std::wstring& ra3Path, const std::wstring& userCommandLine, LanguageData& languageData, HBITMAP customBackground) {
	...
	// BattleNet proxy controls (d3d9 version — DLL goes in game root, not Data/)
	...
	handlers[WM_INITDIALOG] = [&languageData, &icon, &ra3Path, setUpBrushes, initializeCommandLineWindow, adjustButtonFont](HWND window, WPARAM, LPARAM) {
		...
		// BattleNet proxy controls (d3d9 version)
		{
			// Check if proxy DLL exists in game root -> auto-check
			// d3d9.dll is NOT a Known DLL, so it loads from the game
			// root via standard DLL search order on both Windows and Wine.
			auto dllPath = ra3Path + L"Data\\d3d9.dll";
	...
	handlers[WM_CTLCOLOREDIT] = [&commandLineBackground](HWND window, WPARAM hdcAddress, LPARAM childAddress) -> INT_PTR {
		...
		return reinterpret_cast<INT_PTR>(commandLineBackground.get());
	};
	...
	handlers[WM_COMMAND] = [&ra3Path, &languageData, &icon, setUpBrushes, adjustButtonFont, getCommandLines, endControlCenter](HWND window, WPARAM codeAndIdentifier, LPARAM childWindow) {
		...
		switch(identifier) {
			case IDC_ENABLE_PROXY: {
				// Toggle proxy — extract/delete d3d9.dll + Wine registry
				bool checked = (SendMessageW(getControlByID(window, IDC_ENABLE_PROXY), BM_GETCHECK, 0, 0) == BST_CHECKED);
				// d3d9.dll goes in game root (next to RA3.exe), not Data/
				auto dllPath = ra3Path + L"Data\\d3d9.dll";

				if (checked) {
					// Extract d3d9.dll from embedded resource
					auto dllData = Windows::loadBinaryDataResource<char>(
						GetModuleHandle(nullptr), MAKEINTRESOURCEW(PROXY_DLL), (LPCWSTR)(LPCWSTR)RT_RCDATA);
					auto f = Windows::createFile(dllPath, GENERIC_WRITE, 0, CREATE_ALWAYS);
					DWORD written = 0;
					WriteFile(f.get(), dllData.data(), dllData.size(), &written, nullptr);

					// Set Wine registry so Wine prefers our native d3d9
					// (harmless on Windows — key path doesn't exist)
					try {
						auto key = Windows::openRegistryKey(HKEY_CURRENT_USER,
							L"Software\\Wine\\DllOverrides", KEY_WRITE | KEY_READ);
						Windows::setRegistryString(key.get(), L"d3d9", L"native,builtin");
					} catch (...) {
						try {
							...
							if (RegCreateKeyExW(wineKey.get(), L"DllOverrides", 0, nullptr, 0,
								...
								Windows::setRegistryString(hDll, L"d3d9", L"native,builtin");
								RegCloseKey(hDll);
							}
						} catch (...) {}
					}
				} else {
					// Delete DLL + clear Wine registry
					DeleteFileW(dllPath.c_str());
					try {
						auto key = Windows::openRegistryKey(HKEY_CURRENT_USER,
							L"Software\\Wine\\DllOverrides", KEY_WRITE | KEY_READ);
						RegDeleteValueW(key.get(), L"d3d9");
					} catch (...) {}
				}
				break;
			}
...
void aboutWindow(HWND parent, HICON icon, const LanguageData& languageData) {
	...
	auto handlers = ModalDialogBox::HandlerTable{};
	auto font = getNormalFont();
	auto ra3barData = loadBinaryDataResource<DWORD>(GetModuleHandle(nullptr), MAKEINTRESOURCEW(RA3BAR_LOGO), (LPCWSTR)(LPCWSTR)RT_RCDATA);
