...
int main() {
	...
	try {
		...
		for(auto [options, uiFlag] = getInitialUserOptions();
			...
			// d3d9 proxy setup — cross-platform (Windows + Wine)
			if (options->proxyEnabled && !options->proxyDllPath.empty()) {
				// Extract d3d9.dll from embedded resource to game root
				// (next to RA3.exe — DLL search order finds it first on
				//  Windows and on Wine with d3d9=n,b override)
				...
				auto dllPath = ra3Path + L"Data\\d3d9.dll";
				{
					...
					WriteFile(f.get(), dllData.data(), dllData.size(), &total, nullptr)
						>> Windows::checkWin32Result("WriteFile d3d9.dll", Windows::errorValue, false);
				}
				// Write ra3bn.ini only if changed
				{
			...
			// Wine/CrossOver needs WINEDLLOVERRIDES to prefer our native d3d9.
			// Harmless on Windows — the env var is simply ignored.
			if (options->proxyEnabled) {
				SetEnvironmentVariableW(L"WINEDLLOVERRIDES", L"d3d9=n,b");
			}
